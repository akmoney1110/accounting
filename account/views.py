...
class BatchEditView(LoginRequiredMixin, AdminRequiredMixin, View):
    ...
    def get(self, request, pk):
        ...
        # Get current user's unallocated credit for display
        user = batch.user
        txn_type = (
            Transaction.TransactionType.DISBURSEMENT 
            if batch.transaction_type == 'BUY' 
        ...
        total_allocated = PaymentAllocation.objects.filter(
            ...
            transaction__transaction_type=txn_type
        ).exclude(batch=batch).aggregate(total=Sum('amount'))['total'] or Decimal('0.00')
        
        # Available credit that could be applied to this batch
        available_credit = total_paid - total_allocated
    ...
    def post(self, request, pk):
        ...
        if form.is_valid():
            with db_transaction.atomic():
                ...
                additional_payment = form.cleaned_data.get('additional_payment', Decimal('0.00'))
                
                if additional_payment > Decimal('0.00'):
                    # Check if amount exceeds balance due
                    if additional_payment > updated_batch.balance_due:
                        messages.error(
                            request,
                            f"Additional payment ({additional_payment}) exceeds balance due ({updated_batch.balance_due})."
                        )
                        return render(request, self.template_name, {'form': form, 'batch': batch})
                    
                    # Create transaction and allocation
                    trans_type = (
                        Transaction.TransactionType.DISBURSEMENT 
                        if updated_batch.transaction_type == 'BUY' 
                        else Transaction.TransactionType.RECEIPT
                    )
                    
                    transaction = Transaction.objects.create(
                        user=updated_batch.user,
                        batch=updated_batch,
                        amount=additional_payment,
                        transaction_type=trans_type,
                        transaction_date=timezone.now().date(),
                        notes=f"Additional payment on Batch #{updated_batch.batch_code}"
                    )
                    ...
                    messages.success(
                        request,
                        f"Batch updated and additional payment of ₦{additional_payment:,.2f} recorded."
                    )
                else:
...
class BatchDeleteView(LoginRequiredMixin, AdminRequiredMixin, View):
    """Delete a batch and all associated records (ADMIN users only)."""
    
    def get(self, request, pk):
        batch = get_object_or_404(Batch, pk=pk)
        
        # Get summary of associated records for confirmation
        ...
        context = {
            ...
            'total_associated': expense_count + transaction_count + allocation_count,
        }
    ...
    def post(self, request, pk):
        ...
        with db_transaction.atomic():
            ...
            # PaymentAllocations will be deleted via CASCADE if set in models
            # Otherwise, delete them explicitly
            batch.payment_allocations.all().delete()
            batch.expenses.all().delete()
            batch.transactions.all().delete()
            
            # Delete the batch itself
            batch.delete()
        
        messages.success(request, f"Batch {batch_code} and all associated records deleted successfully.")
        return redirect('admin_dashboard')
